...
clear
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/var/lib/HE3ndrixx/data-user-pptp")
	...
	echo -e "==============================="
	grep -E "^### " "/var/lib/HE3ndrixx/data-user-pptp" | cut -d ' ' -f 2-3 | nl -s ') '
...
user=$(grep -E "^### " "/var/lib/HE3ndrixx/data-user-pptp" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^### " "/var/lib/HE3ndrixx/data-user-pptp" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
now=$(date +%Y-%m-%d)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
...
sed -i "s/### $user $exp/### $user $exp4/g" /var/lib/HE3ndrixx/data-user-pptp
clear
echo ""
echo "=========================="
echo "🧑‍💻🥷🎮☁️🎰🌊🛫☁️☁️☁️☁️☁️☁️☁️🏦💵♾️⛽ Cyberpunk☁️   PPTP Account Renewed   "
echo "=========================="
echo "Username  : $user"
echo "Expired   : $exp4"
echo "=========================="
